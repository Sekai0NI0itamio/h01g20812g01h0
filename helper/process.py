...
def process_background_clips_parallel(video_info , blur_background=False, edge_blur=False, max_workers=3):
    ...
    # VideoFileClip instances and nested MoviePy state are not pickle-safe, so
    # threads are the reliable parallelism choice for local rendering.
    logger.info("Using ThreadPoolExecutor for background processing")

    processed_clips = [None] * len(video_info)
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            ...
            for idx, clip_info in enumerate(video_info)
        }
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                processed_clips[idx] = processed_clip
                if processed_clip:
                    logger.info(f"Processed background clip {idx+1}/{len(video_info )}")
            except Exception as e:
                logger.error(f"Failed to process background clip {idx}: {e}")

    total_time = time.time() - start_time
    success_count = len([clip for clip in processed_clips if clip is not None])
    logger.info(f"Processed {success_count}/{len(video_info )} background clips in {total_time:.2f} seconds")
